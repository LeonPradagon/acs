...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "Login berhasil! Mengalihkan ke Admin Panel...",
      });
      ...
      let errorMessage = err.message || "Login gagal. Periksa koneksi atau credential Anda.";
      
      // Handle Axios 401 specifically
      if (err?.response?.status === 401 || errorMessage.includes("401")) {
        ...
        <div className="bg-card/70 dark:bg-[var(--surface-container-low)]/80 backdrop-blur-xl border border-border dark:border-primary/20 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.05)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] p-7 space-y-6 overflow-hidden relative">
          ...
          <div className="text-center flex flex-col items-center">
            <motion.div
              ...
              whileTap={{ scale: 0.95 }}
              className="h-16 w-16 relative overflow-hidden group"
            >
              <Image
                src={appLogo}
                alt={appName}
